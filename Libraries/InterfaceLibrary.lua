...
-- Base64 decoder shim (crypt-only with HttpService fallback)
local b64decode = (crypt and (crypt.base64decode or (crypt.base64 and crypt.base64.decode)))
    or (function(s)
        local ok, res = pcall(function()
            return http:Base64Decode(s)
        end)
...
local library = {
    ...
    images = {
        ['gradientp90'] = 'https://raw.githubusercontent.com/NinjaNathan69/Sammi/refs/heads/main/Libraries/Images/Gradient90.png';
        ['gradientp45'] = 'https://raw.githubusercontent.com/NinjaNathan69/Sammi/refs/heads/main/Libraries/Images/Gradient45.png';
        ['colorhue'] = 'https://raw.githubusercontent.com/NinjaNathan69/Sammi/refs/heads/main/Libraries/Images/ColorHue.png';
        ['colortrans'] = 'https://raw.githubusercontent.com/NinjaNathan69/Sammi/refs/heads/main/Libraries/Images/TransparentVerticalSlider.png';
    };
    numberStrings = {['Zero'] = 0, ['One'] = 1, ['Two'] = 2, ['Three'] = 3, ['Four'] = 4, ['Five'] = 5, ['Six'] = 6, ['Seven'] = 7, ['Eight'] = 8, ['Nine'] = 9};
    signal = loadstring(game:HttpGet('https://raw.githubusercontent.com/NinjaNathan69/Sammi/refs/heads/main/Libraries/Signal.lua'))();
    open = false;
    opening = false;
    hasInit = false;
    cheatname = 'Sammi';
        ...
        function(obj,i,v)
            if not table.find(blacklistedProperties,i) then
                ...
                elseif i == 'Position' and (class == 'Square' or class == 'Image' or class == 'Text') then
                    local parentSizeForPos = drawing.Parent == nil and workspace.CurrentCamera.ViewportSize or drawing.Parent.Object.Size
                    drawing.Object.Position =  utility:UDim2ToVector2(v, parentSizeForPos);
...
function library:init()
    ...
    function self:SaveConfig(name)
        if not self:GetConfig(name) then
            -- Auto-create missing config instead of erroring
            local ok, err = pcall(function()
                writefile(self.cheatname..'/'..self.gamename..'/configs/'..name..self.fileext, http:JSONEncode({}));
            end)
            if ok then
                self:SendNotification('Created new config: '..tostring(name), 3, c3new(0.8,0.8,0));
            else
                self:SendNotification('Error creating config: '..tostring(err)..'. ('..tostring(name)..')', 5, c3new(1,0,0));
                return
            end
        end
    ...
    -- Disable custom cursor (no-op updater)
    local function updateCursor() end
        ...
        if self.hasInit then
            ...
            if library.open then
                ...
                elseif input.UserInputType == Enum.UserInputType.MouseButton2 then
                    for _, wnd in next, library.windows do
                        local dd = wnd.dropdown
                        if dd and dd.selected and dd.objects and dd.objects.background then
                            local bgObj = dd.objects.background.Object
                            if hoverObj == bgObj then
                                local list = dd.selected
                                local max = dd.max or 5
                                local count = #list.values
                                if count > max then
                                    local delta = input.Position.Z > 0 and -1 or 1
                                    dd.start = math.clamp((dd.start or 1) + delta, 1, math.max(1, count - max + 1))
                                    dd:Refresh()
                                end
                                break
                            end
    ...
    utility:Connection(inputservice.InputChanged, function(input, gpe)
        if input.UserInputType == Enum.UserInputType.MouseMovement then
            if library.open then
                ...
                local hoverObj = utility:GetHoverObject();
                if hoverObj ~= lastHoverObj then
                    if lastHoverObj ~= nil then
                        local prevData = library.drawings[lastHoverObj]
                        if prevData then
                            prevData.Hover = false
                            prevData.MouseLeave:Fire(currentMouse)
                        end
                    end
                    if hoverObj ~= nil then
                        local newData = library.drawings[hoverObj]
                        if newData then
                            newData.Hover = true
                            newData.MouseEnter:Fire(currentMouse)
                        end
                    end
    ...
    function self.UpdateThemeColors()
        for _,v in next, library.drawings do
            ...
            if v.OutlineThemeColor and library.theme[v.OutlineThemeColor] then
                v.Object.OutlineColor = utility:AddRGB(library.theme[v.OutlineThemeColor],fromrgb(v.OutlineThemeColorOffset,v.OutlineThemeColorOffset,v.OutlineThemeColorOffset))
            end
        end
    end
    ...
    -- Global keybind conflict detection
    function self:FindBindConflict(requester, key)
        if key == nil or key == 'none' then return nil end
        for _, b in next, self.binds do
            if b ~= requester and b.bind ~= 'none' and b.bind == key then
                return b
            end
        end
        return nil
    ...
    function self.NewIndicator(data)
        local indicator = {
            ...
            lock = data.lock or 'custom', -- supports 'Right Center'
            xoffset = data.xoffset or 0,
            yoffset = data.yoffset or 0,
            values = {},
            objects = {valueObjects = {}},
            spacing = '   ',
            matchWatermark = data.matchWatermark or false,
        };
        ...
        function indicator:Update()
            ...
            local targetWidth = xSize + 8
            if self.matchWatermark and library.watermark and library.watermark.objects and library.watermark.objects.background then
                local wmSize = library.watermark.objects.background.Object.Size
                if wmSize and wmSize.X and wmSize.X > 0 then
                    targetWidth = wmSize.X
                end
            end
            self.objects.background.Size = newUDim2(0,targetWidth,0,16)
            if self.lock == 'Right Center' then
                local screensize = workspace.CurrentCamera.ViewportSize
                local size = self.objects.background.Object.Size
                local y = math.floor(screensize.Y/2 + self.yoffset)
                local x
                -- Prefer aligning to watermark's right edge for perfect match
                local wm = library.watermark
                if wm and wm.objects and wm.objects.background then
                    local wmObj = wm.objects.background.Object
                    local wmPos = wmObj.Position
                    local wmSize = wmObj.Size
                    x = wmPos.X + wmSize.X - size.X
                else
                    x = screensize.X - size.X - 15
                end
                self.objects.background.Position = newUDim2(0, x + self.xoffset, 0, y)
            else
    ...
    function self.NewWindow(data)
        ...
        do
            ...
            do
                ...
                -- Procedural smooth vertical gradient (no checkerboard, no banding)
                objs.transLines = {}
                for i = 0, 174 do
                    local line = utility:Draw('Square', {
                        Size = newUDim2(1,0,0,1);
                        Position = newUDim2(0,0,0,i);
                        Color = c3new(1,1,1);
                        Transparency = 1 - (i/174); -- top opaque -> bottom transparent
                        ZIndex = z+3;
                        Parent = objs.transColor;
                    })
                    table.insert(objs.transLines, line)
                end
                ...
                local function updateTrans(pos)
                    if window.colorpicker.selected ~= nil then
                        Y = (objs.transColor.Object.Position.Y + objs.transColor.Object.Size.Y) - objs.transColor.Object.Position.Y
            ...
            function window.colorpicker:Visualize(c3, a)
                ...
                -- Update alpha bar preview color on each line for a smooth gradient
                if self.objects.transLines then
                    local baseColor = fromhsv(h,s,v)
                    for _, line in ipairs(self.objects.transLines) do
                        line.Color = baseColor
                    end
                else
                    -- Fallback: tint the underlay if lines are missing
        ...
        do
            ...
            function window.dropdown:Refresh()
                if self.selected ~= nil then
                    ...
                    local start = self.start or 1
                    local max = self.max or 5
                    if start < 1 then start = 1 end
                    if #list.values > 0 then
                        start = math.clamp(start, 1, math.max(1, #list.values - max + 1))
                        self.start = start
                    else
                        self.start = 1
                    end
                    ...
                    for idx, obj in next, self.objects.values do
                        ...
                        local visible = (valueStr ~= nil) and (idx >= start and idx < start + limit)
                    ...
                    -- Ensure container height fits only the visible rows
                    self.objects.background.Size = newUDim2(1,-6,0, math.max(2, (shown > 0 and (2 + shown * (18 + padding) - padding) or 20)));
        ...
        function window:AddTab(text, order)
            ...
            function tab:AddSection(text, side, order)
                ...
                function section:AddToggle(data)
                    ...
                    function toggle:AddBind(data)
                        ...
                        table.insert(library.binds, bind)
                        ...
                        function bind:SetBind(keybind)
                            ...
                            if self.bind == Enum.KeyCode.Backspace then
                                ...
                                if self.bind ~= 'none' and tostring(keyName):upper() ~= 'NONE' then
                                    ...
                                    if conflict then
                                        library:SendNotification("Keybind '"..tostring(keyName):upper().."' was in use. Unbinding previous.", 3)
                                        pcall(function() conflict:SetBind(Enum.KeyCode.Backspace) end)
                                    end
                                end
                            end
            ...
            function tab:Select()
                local prev = window.selectedTab
                window.selectedTab = tab;
                window:UpdateTabs();
                -- Animate all sections of the newly selected tab (staggered slide + fade)
                local idx = 0
                for _, section in next, tab.sections do
                    if section.enabled then
                        idx = idx + 1
                        local bgProxy = section.objects.background
                        local bgObj = bgProxy.Object
                        local targetPos = bgProxy.Position
                        task.spawn(function()
                            task.wait((idx-1) * 0.03)
                            pcall(function()
                                -- start slightly above target using UDim2 offset
                                bgProxy.Position = newUDim2(targetPos.X.Scale, targetPos.X.Offset, targetPos.Y.Scale, targetPos.Y.Offset - 8)
                                bgObj.Transparency = 0.05
                            end)
                            utility:Tween(bgProxy, 'Position', targetPos, 0.22, nil, Enum.EasingStyle.Sine)
                            utility:Tween(bgObj, 'Transparency', 1, 0.22, nil, Enum.EasingStyle.Sine)
                        end)
                    end
                end
    ...
    utility:Connection(runservice.RenderStepped, function(step)
        ...
        -- Throttle ping sampling to match watermark updates
    ...
    -- Move Keybinds indicator to middle-right aligned to the same right margin as the watermark
...
themeSection:AddList({text = 'Presets', flag = 'preset_theme', values = themeStrings, callback = function(newTheme)
    ...
    -- Order: Background > Text > Border > Rest, and inside each group: longest to shortest name
    do
        local keys = {}
        for k,_ in pairs(library.theme) do table.insert(keys, k) end

        local function groupRank(name)
            local n = string.lower(name)
            if string.find(n, 'background') then return 1 end
            if string.find(n, 'text') then return 2 end
            if string.find(n, 'border') then return 3 end
            return 4
        end
        ...
        table.sort(keys, function(a,b)
            ...
            -- longest to shortest within group
        ...
        for _, key in ipairs(keys) do
            themeSection:AddColor({text = key, flag = key, color = library.theme[key], callback = function(c3)
...
-- PascalCase public API facade (non-breaking)
-- Expose a PascalCase proxy that forwards to the existing `library` table.
-- This keeps backward compatibility while allowing external scripts to use PascalCase names.
 do
     local map = {
        -- core tables/fields
        Windows = 'windows',
     ...
     -- classify functions to preserve calling conventions
     local methodFuncs = {
         Unload = true,
         Init = true,
         CreateSettingsTab = true,
         SetTheme = true,
         GetConfig = true,
         LoadConfig = true,
         SaveConfig = true,
     ...
     LibraryProxy = setmetatable({}, {
         __index = function(_, k)
             ...
             if type(v) == 'function' then
                 if staticFuncs[key] then
                     -- forward as-is for static (dot-style) functions
                     return function(...)
                         return v(...)
                     end
                 else
                     -- wrap to accept both colon and dot calls; ensure `library` is self
                     return function(...)
                         local args = {...} 
                         if args[1] == LibraryProxy then
                             table.remove(args, 1)
                         end
                         return v(library, table.unpack(args))
                     end
                 end
             end
             return v
         end,
         __newindex = function(_, k, vv)
             local key = map[k] or k
             library[key] = vv
         end,
         __tostring = function()
             return 'Library(PascalCase facade)'
         end
     })

    getgenv().Library = LibraryProxy
end
